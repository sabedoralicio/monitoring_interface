...
function isNoHelpIndex(responseString:string): boolean {
  return responseString.indexOf("Sorry, the help index cannot be opened. ") == 0;
}
...
function isSLIFunctionDictError(responseString:string): boolean {
  return responseString.indexOf("DictError in SLI function SLIInterpreter") == 0;
}
...
function asNestResponseLore(responseString:string): NestResponseLore {
  if(isCallableSignatureNotSupported(responseString)) {
  	return { kind: NestResponseKind.CallableSignatureNotSupported, code: responseString };
  	...
  	return { kind: NestResponseKind.SLIFunctionDictError, code: responseString };
